...
import { useGetLeaderboardData } from '@workspace/api-client-react';
...
export default function Leaderboard() {
  const { data: leaderboardRes, isLoading } = useGetLeaderboardData({
        ...
        <div className="bg-slate-900 text-white p-4 grid grid-cols-12 gap-4 font-semibold text-sm">
          ...
          <div className="divide-y">
            {leaderboardData.map((entry: any, idx: number) => {
              const name = entry?.studentName || entry?.name || entry?.username || entry?.user?.name || entry?.student?.name || 'Student';
              const rank = entry?.rank || idx + 1;
              const university = entry?.university || entry?.student?.university || 'University';
              const semester = entry?.semesterNumber || entry?.currentSemester || 1;
              const score = entry?.ppsScore ?? entry?.creditScore ?? 0;
              const key = entry?.studentId || entry?._id || idx;

              return (
                <div key={key} className={`p-4 grid grid-cols-12 gap-4 items-center transition-colors hover:bg-muted/50 ${idx < 3 ? 'bg-amber-50/30 dark:bg-amber-900/10' : ''}`}>
                  <div className="col-span-1 flex justify-center">
                    {idx === 0 ? <Medal className="w-6 h-6 text-yellow-500" /> :
                     ...
                     <span className="font-mono text-muted-foreground font-semibold">{rank}</span>}
                  </div>
                  ...
                  <div className="col-span-5 flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <div className="text-xs text-muted-foreground">Semester {semester}</div>
                    </div>
                  </div>
                  ...
                  <div className="col-span-3 flex justify-end items-center gap-4">
                    ...
                    <div className="font-display font-bold text-xl text-primary flex items-center gap-1.5">
                      {score} <Target className="w-4 h-4 opacity-50" />
                    </div>
                  </div>
                </div>
              );
